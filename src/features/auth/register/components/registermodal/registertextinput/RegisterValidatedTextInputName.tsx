...
export const RegisterValidatedTextInputName:React.FC<RegisterValidatedTextInputsProps> = ({valueName, value,maxLength}) => {
  ...
  return (
    <div className='register-validated-text-input-container'>
        <div className='register-validated-text-input-content'>
          <ValidatedTextInput 
          valid={nameValid} name={valueName} label = {valueName.charAt(0).toUpperCase()+valueName.substring(1)}
          changeValue={updateValue} data={value} attributes={{maxLength: maxLength}} />
          {nameValid ? <></> : <span className='register-validated-text-input-error'>Please enter your {valueName.charAt(0).toUpperCase()+valueName.substring(1)}</span>}
        </div>
         </div>
  )
}
